package com.chen.auth.server.interceptor;/**
 * Created by Administrator on 2019/9/9.
 */

import com.chen.auth.api.enums.StatusCode;
import com.chen.auth.api.response.BaseResponse;
import com.chen.auth.server.service.CommonService;
import com.chen.auth.server.service.JwtTokenRedisService;
import com.chen.auth.server.service.JwtTokenService;
...
public class JwtTokenRedisInterceptor implements HandlerInterceptor{
    ...
    private static final Logger log= LoggerFactory.getLogger(JwtTokenRedisInterceptor.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (handler instanceof HandlerMethod){
            ...
            if (StringUtils.isBlank(accessToken)){
                ...
                log.info("--jwt+redis用户验证-开始进行--");

                BaseResponse result=jwtTokenRedisService.validateToken(accessToken);
